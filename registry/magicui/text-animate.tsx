...
const TextAnimateBase = ({
  ...
  } else if (animation && defaultItemAnimationVariants[animation]) {
    const anim = defaultItemAnimationVariants[animation];
    finalVariants = {
      container: {
        ...anim.container,
        show: {
          ...(typeof anim.container.show === 'object' ? anim.container.show : {}),
          transition: {
            delayChildren: delay,
            staggerChildren: duration / Math.max(segments.length, 1),
          },
        ...
        exit: {
          ...(typeof anim.container.exit === 'object' ? anim.container.exit : {}),
          transition: {
            staggerChildren: duration / Math.max(segments.length, 1),
            staggerDirection: -1,
          },
